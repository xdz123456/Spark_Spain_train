...
def execute(dirName):
    ...
    rf = RandomForestClassifier(labelCol="indexedLabel", featuresCol="indexedFeatures", numTrees=50, maxDepth=30)
    ...
    # predictions.select("prediction", "indexedLabel", "features").toPandas().to_csv('resultRF.csv')
    ...
    data=open("./"+dirName+"/AccerancyRF.txt",'w+')
    print("Accerancy %f\n" %accuracy, file = data)
    data.close()
